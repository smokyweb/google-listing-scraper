...
export default function EmailCampaign() {
  ...
  const [maxPerSend, setMaxPerSend] = useState(25);
  ...
  useEffect(() => {
    ...
    // Load email quota
    apiFetch('/email/quota').then(setEmailQuota).catch(() => {});
  }, []);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const limit = selectedIds.size > 0 ? Math.min(selectedIds.size, maxPerSend) : maxPerSend;
      const data = await apiFetch('/email/send', { method:'POST', body: JSON.stringify({ subject, body, leadIds: selectedIds.size > 0 ? [...selectedIds].slice(0, limit) : undefined, senderId: selectedSenderId || undefined, replyToEmail, maxPerSend: limit }) });
      setResult(data);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          ...
          <div className="bg-gray-800/60 rounded-lg p-4 space-y-3">
            <h4 className="text-sm font-medium text-white">Sending Limits</h4>
            <div className="flex items-center gap-3">
              <label className="text-xs text-gray-400 shrink-0">Max per send:</label>
              <input type="number" min="1" max="25" value={maxPerSend} onChange={e => setMaxPerSend(Math.min(25, Math.max(1, parseInt(e.target.value) || 1)))}
                className="w-20 px-2 py-1 bg-gray-800 border border-gray-700 rounded text-white text-xs focus:outline-none focus:border-blue-500" />
              <span className="text-xs text-gray-600">/ 25 max</span>
            </div>
            {emailQuota && (
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-400">Today:</span>
                <span className={`text-xs font-medium ${emailQuota.remaining > 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {emailQuota.sentToday}/{emailQuota.dailyCap} sent
                </span>
                <span className="text-xs text-gray-600">|</span>
                <span className={`text-xs font-medium ${emailQuota.remaining > 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {emailQuota.remaining} remaining
                </span>
              </div>
            )}
          </div>
