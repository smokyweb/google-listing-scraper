...
router.get('/quota', authMiddleware, (req, res) => {
  ...
  const emailsSentToday = db.prepare(
    "SELECT COUNT(*) as count FROM leads WHERE email_status = 'sent' AND date(email_sent_at) = ?"
  ).get(today).count;
  const DAILY_CAP = 50;
  res.json({ sentToday: emailsSentToday, remaining: DAILY_CAP - emailsSentToday, dailyCap: DAILY_CAP });
});
...
router.post('/send', authMiddleware, async (req, res) => {
  try {
    const { subject, body, leadIds, maxPerSend } = req.body;
    if (!subject || !body) return res.status(400).json({ error: 'subject and body are required' });

    // --- DAILY CAP CHECK (server-side enforcement) ---
    ...
    const DAILY_CAP = 50;
    const PER_SEND_CAP = Math.min(25, maxPerSend || 25);
    if (emailsSentToday >= DAILY_CAP) {
      return res.status(429).json({ error: 'Daily email limit reached', emailsSentToday, dailyCap: DAILY_CAP });
    }
    const remainingToday = DAILY_CAP - emailsSentToday;
    // --- PER-SEND CAP ---
    let maxCount = Math.min(PER_SEND_CAP, remainingToday);
